...
//  Playtest
//
//  Created by Bart van den Berg 04/18/16
//
//

#import "PlaytestPlugin.h"
#import "PTConnect.h"

@implementation PlaytestPlugin
...
- (void) setAppVersion:(CDVInvokedUrlCommand*)command
{
    [self.commandDelegate runInBackground:^{
        ...
        NSLog(@"Setting Playtest version to %@", [command.arguments objectAtIndex:0]);
...
- (void) startSession:(CDVInvokedUrlCommand*)command
{
    [self.commandDelegate runInBackground:^{
        CDVPluginResult* pluginResult = nil;
        NSString* feedbackKey = [command.arguments objectAtIndex:0];
        NSString* apiKey = [command.arguments objectAtIndex:1];

        // NSString* javaScript = nil;
        
        NSLog(@"Starting Playtest Session with Feedback key %@ and apiKey: %@", feedbackKey,apiKey);
        
        @try {
            if (feedbackKey != nil && apiKey != nil) {
                dispatch_sync(dispatch_get_main_queue(), ^{[[PTConnect manager] setupFeedbackID:feedbackKey withApiKey:apiKey];});
                pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
                // starting Playtest - Update Key before releasing
            }
        }
        @catch (NSException *exception) {
